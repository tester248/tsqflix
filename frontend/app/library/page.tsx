    ...
    <div className="container py-8 space-y-12">
      <section className="space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold uppercase tracking-widest">History</h2>
        </div>
        
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 rounded-3xl border border-dashed border-white/10 text-zinc-500">
            <p className="text-sm">Movies and shows you watch will appear here.</p>
          </div>
        ) : (
            ...
            {history.map((item: any, idx) => (
              <div key={`${item.id}-${idx}`} className="relative group">
                ...
                <button 
                  ...
                  className="absolute top-2 right-2 p-1.5 rounded-lg bg-black/60 text-white/50 hover:text-rose-500 hover:bg-black opacity-0 group-hover:opacity-100 transition-all backdrop-blur-md border border-white/10"
                >
